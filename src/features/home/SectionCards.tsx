...
export default function SectionCards() {
  ...
  const renderCard = (c: { title: string; desc: string; to: string; image: string}) => (
    <Link
      key={c.title}
      to={c.to}
      className="group/card relative flex -4 h-56 w-full overflow-hidden rounded-2xl border border-[color:var(--battleship-gray)]/35 bg-black/5 shadow-[0_10px_30px_rgba(0,0,0,0.04)] transition-transform transition-shadow hover:-translate-y-1 hover:shadow-[0_16px_40px_rgba(0,0,0,0.12)] md:h-64"
    >
      <img
        src={c.image}
        alt=""
  ...
  return (
    <div id="sections" className="flex flex-col gap-8 py-4 group h-full w-full px-4">
